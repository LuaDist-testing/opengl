...
static int luaglu_scaleimage(lua_State *L)
{
  ...
  widthout = (GLint)luaL_checkinteger(L, 5);
  heightout = (GLint)luaL_checkinteger(L, 6);
  sizeout = widthout*heightout*depth;

  pixelsout = LUAGL_NEW_ARRAY(GLfloat, sizeout);

  error = gluScaleImage(format, (GLint)luaL_checkinteger(L, 2), (GLint)luaL_checkinteger(L, 3), GL_FLOAT, pixelsin,
...
{
  lua_pushnumber(L, gluScaleImage(luagl_get_gl_enum(L, 1), (GLint)luaL_checkinteger(L, 2), (GLint)luaL_checkinteger(L, 3), luagl_get_gl_enum(L, 4), luagl_checkuserdata(L, 5),
    (GLint)luaL_checkinteger(L, 6), (GLint)luaL_checkinteger(L, 7), luagl_get_gl_enum(L, 8), luagl_checkuserdata(L, 9)));
  return 1;
}
...
static int luaglu_build_mipmaps(lua_State *L)
{
  ...
  components = (GLint)luaL_checkinteger(L, 1);
  ...
  if (height != -1)
  {
    lua_pushnumber(L, gluBuild2DMipmaps(GL_TEXTURE_2D, components, width/components, height, 
                 luagl_get_gl_enum(L, 2), GL_FLOAT, pixels));
  }
  else
  {
    /* if not 2D, get 1D */
...
{
  lua_pushnumber(L, gluBuild1DMipmaps(GL_TEXTURE_1D, (GLint)luaL_checkinteger(L, 1), (GLint)luaL_checkinteger(L, 2),
...
static int luaglu_build_2d_mipmaps(lua_State *L)
{
  if (lua_isnumber(L, 1))
  {
    lua_pushnumber(L, gluBuild2DMipmaps(GL_TEXTURE_2D, (GLint)luaL_checkinteger(L, 1), (GLint)luaL_checkinteger(L, 2), (GLint)luaL_checkinteger(L, 3),
  ...
  else
  {
    ...
    h = luagl_get_array2f(L, 1, &pixels, &w);
    if (h==-1)
    ...
    lua_getfield(L, 1, "components");  components = (GLint)luaL_checkinteger(L, -1);  lua_remove(L, -1);
    ...
    lua_pushnumber(L, gluBuild2DMipmaps(target, components, width, height, format, type, pixels));
...
static int luaglu_cylinder(lua_State *L)
{
  ...
  gluCylinder(lquad->quad,luaL_checknumber(L, 2), luaL_checknumber(L, 3), luaL_checknumber(L, 4),
              (GLint)luaL_checkinteger(L, 5), (GLint)luaL_checkinteger(L, 6));
...
static int luaglu_disk(lua_State *L)
{
  ...
  gluDisk(lquad->quad,luaL_checknumber(L, 2), luaL_checknumber(L, 3),
          (GLint)luaL_checkinteger(L, 4), (GLint)luaL_checkinteger(L, 5));
...
{
  LuaGLUquadric *lquad=luaglu_checkquadric(L,1);
  gluSphere(lquad->quad, luaL_checknumber(L, 2), (GLint)luaL_checkinteger(L, 3), (GLint)luaL_checkinteger(L, 4));
  lua_pushvalue(L,1);
  return 1;
}
...
static void nurbVertexDataCB( GLfloat *vertex, void *userData ) 
{
  ...
  lua_pushvalue(L,1); /* push nurb user data */
  luagl_push_arrayf(L, vertex, 3);  /* All the generated vertices have dimension 3 */
  lua_call(L,2,0); /* call the handler */
  lua_pop(L,1); /*remove cb table*/
  return;
}
...
static void nurbColorDataCB( GLfloat *color, void *userData )
{
  ...
  /* assume udata at satck index 1 */
  lua_pushvalue(L,1); /* push nurb user data */
  luagl_push_arrayf(L, color, 4);  /* RGBA */
  lua_call(L,2,0); /* call the handler */
  lua_pop(L,1); /*remove cb table*/
  return;
};
...
static void nurbTexCoordDataCB( GLfloat *tex_coord, void *userData )
{
  ...
  /* assume udata at satck index 1 */
  lua_pushvalue(L,1); /* push nurb user data */
  luagl_push_arrayf(L, tex_coord, 4);  /* can be 1,2,3 or 4 depending on which type of texture map is specified (????) */
  lua_call(L,2,0); /* call the handler */
  lua_pop(L,1); /*remove cb table*/
  return;
}
...
static int luaglu_nurbs_callback(lua_State *L) 
{
  ...
  case GLU_NURBS_END : 
    ...
    break;
  default : 
    luaL_argerror(L, 2, "invalid or unknown enumeration");
  }
...
int luaopen_luaglu(lua_State *L)
{
  ...
  lua_pushvalue(L,-2);
  lua_settable(L,-3);
  luagl_register_funcs(L, luagluquadric_methods);
  ...
  lua_pushvalue(L,-2);
  lua_settable(L,-3);
  luagl_register_funcs(L, luaglunurb_methods);

  luagl_register_lib(L, "glu", luaglu_lib);
