...
#define LUAGL_VERSION "1.11"


#if LUA_VERSION_NUM != 502  /* unsigned support existed only in Lua 5.2 */
...
static int luagl_are_textures_resident(lua_State *L)
{
  ...
  if (glAreTexturesResident(n, textures, residences))
    memset(residences, 1, sizeof(GLboolean) * n);
...
{
  GLsizei size = luaL_checkinteger(L, 1);
  GLfloat *buffer = LUAGL_NEW_ARRAY(GLfloat, size);
...
static int luagl_free_feedback_buffer(lua_State *L)
{
...
{
  GLubyte *mask = LUAGL_NEW_ARRAY(GLubyte, 1024);  /* 32x32 */
...
static int luagl_polygon_stipple(lua_State *L)
{
  ...
  if (height == -1)  /* if not 2D, get 1D */
  {
    ...
    {
      LUAGL_DELETE_ARRAY(parray);
      luaL_argerror(L, 1, "invalid stipple size (width x height != 32 x 32)");
    }
...
static int luagl_set_data_value(lua_State *L)
{
  ...
  case GL_INT:
    {
      GLint* data = (GLint*)buffer;
      data[index] = (GLint)luaL_checkinteger(L, 4);
      break;
    }
  case GL_UNSIGNED_INT:
    {
  ...
  case GL_DOUBLE:
    {
      GLdouble* data = (GLdouble*)buffer;
      data[index] = (GLdouble)luaL_checknumber(L, 4);
      break;
    }
  }
...
static int luagl_tex_image(lua_State *L)
{
  ...
  border = luaL_optinteger(L, index + 1, 0);
  ...
  {
    width = luagl_get_arrayf(L, index, &pixels);
    glTexImage1D(GL_TEXTURE_1D, luaL_checkinteger(L, 1),
                 iformat, width/depth, border, format, GL_FLOAT, pixels);
  }
  LUAGL_DELETE_ARRAY(pixels);
  return 0;
}
...
static const luaL_Reg luagl_lib[] = {
  ...
  {"FreeFeedbackBuffer", luagl_free_feedback_buffer},
  {"Finish", luagl_finish},
  {"Flush", luagl_flush},
  {"Fog", luagl_fog},
  ...
  {"SetDataValue", luagl_set_data_value},
  {"GetDataValue", luagl_get_data_value},
  {"SetData", luagl_set_data},
  {"GetData", luagl_get_data},
...
{
  luagl_register_lib(L, "gl", luagl_lib);
